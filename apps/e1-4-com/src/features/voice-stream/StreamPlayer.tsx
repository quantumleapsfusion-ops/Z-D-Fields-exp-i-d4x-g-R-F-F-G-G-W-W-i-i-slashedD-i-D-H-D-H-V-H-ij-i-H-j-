...
>(function StreamPlayer({ segments, audioUrl, onActiveChange }, ref) {
  ...
  return (
    <div className="border-chalk/15 bg-blackboard/90 flex items-center gap-4 rounded-full border px-4 py-2 backdrop-blur">
      <audio
        ref={audio}
        preload="auto"
      ...
      <div className="relative flex-1">
        <div className="pointer-events-none absolute inset-y-0 right-0 left-0 flex items-center">
          {segments.map((s, i) => (
            <span
              key={s.id}
              className={`border-blackboard h-1 border-r ${i === current ? "bg-ochre/70" : "bg-chalk/20"}`}
