    ...
    <PageShell>
      <section className="mx-auto max-w-3xl px-5 py-16 sm:px-8">
        <div className="flex items-center gap-6">
          ...
          />
          <div>
            <h1 className="font-display text-4xl font-bold tracking-display">
              {user.name ?? 'Earthling'}
            </h1>
            <p className="mt-1 font-sans text-sm text-dust">{user.email}</p>
            <p className="label mt-2">{sessionUser.providers.join(' · ') || 'Supabase Auth'}</p>
          </div>
        </div>
        ...
        <h2 className="label mb-3">Destroy all my voice data</h2>
        <p className="mb-4 max-w-xl text-sm leading-relaxed text-chalk/70">
          Permanently deletes every Voice Stream segment: the audio files, their transcriptions,
          derived Da Vinci output and every share link. Your account, name and avatar stay. Primary
          storage is wiped immediately; backups within the window stated in our{' '}
        ...
        <h2 className="label mb-3">Delete account</h2>
        <p className="mb-4 max-w-xl text-sm leading-relaxed text-chalk/70">
