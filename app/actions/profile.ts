...
import { AVATARS_BUCKET, getStorage, storageKeys } from '@/lib/storage';
...
export async function uploadAvatar(_prev: ActionState, form: FormData): Promise<ActionState> {
  ...
  await storage.put(AVATARS_BUCKET, key, new Uint8Array(await file.arrayBuffer()), file.type);
  ...
  if (previous?.avatarKey) await storage.delete(AVATARS_BUCKET, previous.avatarKey);
...
export async function removeAvatar(): Promise<void> {
  ...
  if (user?.avatarKey) {
    await getStorage().delete(AVATARS_BUCKET, user.avatarKey);
