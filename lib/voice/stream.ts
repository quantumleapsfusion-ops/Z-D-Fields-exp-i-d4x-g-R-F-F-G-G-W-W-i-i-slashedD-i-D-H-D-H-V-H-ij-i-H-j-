...
import { getStorage, storageKeys, VOICE_BUCKET } from '@/lib/storage';
...
export async function appendSegment(input: AppendInput): Promise<VoiceSegment> {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    await getStorage().put(VOICE_BUCKET, audioKey, input.audio, input.mimeType);
    try {
      ...
      await getStorage().delete(VOICE_BUCKET, audioKey);
...
export async function transcribeSegment(segmentId: string): Promise<void> {
  ...
  if (!segment) return;
  try {
    const blob = await getStorage().get(VOICE_BUCKET, segment.audioKey);
