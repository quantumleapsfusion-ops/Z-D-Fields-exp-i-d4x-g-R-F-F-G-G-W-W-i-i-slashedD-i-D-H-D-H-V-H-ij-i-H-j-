...
export function GravityBoard({ llmReady }: { llmReady: boolean }) {
  ...
  useEffect(() => {
    if (phase === 'gathering' && d >= EVENT_HORIZON && live.phrases.length > collapsedAt.current) {
      void collapse();
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      {!llmReady ? (
        <p className="rounded-xl border border-ochre/30 bg-ochre/5 px-4 py-3 font-sans text-sm text-chalk/80">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative h-2 flex-1 rounded-full bg-chalk/10">
          <div
            className="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-dust to-ochre transition-all duration-500"
            style={{ width: `${Math.round(d * 100)}%` }}
          />
          <div
            className="absolute -top-1.5 h-5 w-px bg-chalk"
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="relative h-[26rem] overflow-hidden rounded-2xl border border-chalk/10 bg-[radial-gradient(circle_at_center,#000_0,#0e1a13_60%)]">
          ...
          <span className="label absolute bottom-3 right-3">3D mock-up · not 4D</span>
        </div>

        <div className="flex flex-col gap-3">
          {phase === 'gathering' || phase === 'collapsing' ? (
            <div className="flex h-full flex-col justify-between gap-4">
              <p className="font-display text-xl leading-snug text-chalk/85">
              ...
              <button
                ...
                className="self-start rounded-full border border-chalk/25 px-5 py-2 font-sans text-sm hover:border-ochre hover:text-ochre disabled:opacity-40"
              >
            ...
            <>
                    ...
                    <button
                      ...
                      className={`rounded-xl border p-4 text-left transition-all duration-700 ${
                        isChosen
                          ...
                          : faded
                            ? 'scale-95 border-chalk/5 opacity-25 blur-[1px]'
                            : 'animate-shimmer border-chalk/15 hover:border-chalk/40'
                      }`}
                      style={
                        phase === 'superposed' ? { animationDelay: `${i * 400}ms` } : undefined
                      }
                    >
                      ...
                      <span className="mt-2 block font-display text-lg">{c.title}</span>
                      <span className="mt-1 block font-sans text-sm text-chalk/70">
                        {c.interpretation}
                      </span>
                    </button>
                  );
              ...
              <div className="flex flex-wrap gap-3">
                {phase === 'superposed' && superposition ? (
                  <button
                    ...
                    className="rounded-full bg-chalk px-5 py-2 font-sans text-sm text-blackboard"
                  >
                ...
                <button
                  type="button"
                  onClick={() => void collapse()}
                  className="rounded-full border border-chalk/25 px-5 py-2 font-sans text-sm hover:border-ochre"
                >
                  Sample again
                </button>
              ...
              <p className="label">
                Source: {superposition?.source === 'llm' ? 'LLM (temperature 1.0)' : 'random stub'}
              </p>
            </>
          )}
          {error ? <p className="font-sans text-sm text-ochre">{error}</p> : null}
        </div>
      </div>
    </div>
  );
