    ...
    <div className="flex flex-col gap-8">
      {!llmReady ? (
        <p className="rounded-xl border border-ochre/30 bg-ochre/5 px-4 py-3 font-sans text-sm text-chalk/80">
          No LLM key configured — transcription works, and the canvas runs a small keyword sketcher
          (try “sun over the mountains and the sea”). Add <code>ANTHROPIC_API_KEY</code> or{' '}
          <code>OPENAI_API_KEY</code> to let Da Vinci draw anything you describe.
        </p>
      ) : null}
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,5fr)_minmax(0,7fr)]">
        ...
        <figure className="flex flex-col gap-2">
          <div className="chalk-surface overflow-hidden rounded-2xl border border-chalk/10">
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-chalk/10 pt-6">
        ...
        {LANGUAGES.map((lang) => (
          <button
            ...
            className="rounded-full border border-chalk/15 px-3 py-1 font-sans text-xs text-chalk/80 transition-colors hover:border-ochre hover:text-ochre disabled:cursor-not-allowed disabled:opacity-40"
          >
            {lang}
          </button>
        ))}
        <button
          type="button"
          onClick={clearAll}
          className="ml-auto rounded-full px-3 py-1 font-sans text-xs text-dust hover:text-chalk"
        >
          Clear
        </button>
      </div>
      {translateError ? <p className="font-sans text-sm text-ochre">{translateError}</p> : null}
      {translation ? (
        <div className="animate-ink-in">
          <p className="label mb-2">{translation.lang}</p>
          <p className="whitespace-pre-line font-display text-2xl leading-snug">
            {translation.text}
          </p>
        </div>
      ) : null}
