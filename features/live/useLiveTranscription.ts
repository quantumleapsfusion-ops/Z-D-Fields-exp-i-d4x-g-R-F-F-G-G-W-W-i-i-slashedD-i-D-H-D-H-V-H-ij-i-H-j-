 ...
 */
export function useLiveTranscription(onPhrase?: (phrase: FinalPhrase) => void): LiveTranscription {
  const [listening, setListening] = useState(false);
  const [provider, setProvider] = useState<LiveTranscriptionConfig['provider'] | null>(null);
  ...
  useEffect(() => {
    ...
    const hasMedia = Boolean(navigator.mediaDevices?.getUserMedia) && 'MediaRecorder' in window;
    setSupported(hasWebSpeech || hasMedia);
  ...
  const startBrowser = useCallback(() => {
    ...
    if (!Ctor) throw new Error('This browser has no built-in speech recognition. Type instead.');
  ...
  return { listening, provider, supported, phrases, interim, error, start, stop, pushText, reset };
}
