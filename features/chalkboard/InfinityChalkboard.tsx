...
import { useLiveTranscription, type FinalPhrase } from '@/features/live/useLiveTranscription';
...
const COLORS = ['#f1ede1', '#93a294', '#d3a34c', '#e8857a', '#7fb7d9', '#9ccf8f', '#e9d25a'];

type History = { past: BoardElement[][]; present: BoardElement[]; future: BoardElement[][] };
type HistoryAction =
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      setBoards((prev) => {
        ...
        return [
          { id, title: cur.title || 'Untitled board', updatedAt: new Date().toISOString() },
          ...rest,
        ];
      });
      setSaveState('saved');
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-3">
        <input
          ...
          className="min-w-0 flex-1 bg-transparent font-display text-2xl text-chalk focus:outline-none"
        />
        ...
        <select
          ...
          className="h-9 rounded-full border border-chalk/15 bg-blackboard px-3 font-sans text-sm text-chalk"
        >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div
          ...
          className="flex rounded-full border border-chalk/15 p-1"
        >
          {(
            [
            ...
            <button
              ...
              className={`rounded-full px-4 py-1 font-sans text-sm transition-colors disabled:cursor-not-allowed disabled:opacity-35 ${
                mode === m.id ? 'bg-chalk text-blackboard' : 'text-chalk/80 hover:text-chalk'
              }`}
            >
              {m.label}
            </button>
            ...
            {TOOLS.map((t) => (
              <button
                ...
                className={`rounded-full px-3 py-1 font-sans text-xs transition-colors ${
                  tool === t.id ? 'bg-ochre text-blackboard' : 'text-chalk/75 hover:text-chalk'
                }`}
              >
                {t.label}
              </button>
            ))}
            <span className="mx-2 h-5 w-px bg-chalk/15" aria-hidden="true" />
            {COLORS.map((c) => (
              <button
                key={c}
                type="button"
                ...
                onClick={() => setColor(c)}
                className={`h-5 w-5 rounded-full border ${color === c ? 'border-chalk ring-2 ring-ochre/60' : 'border-chalk/20'}`}
      ...
      <div className="chalk-surface relative h-[70vh] min-h-[28rem] overflow-hidden rounded-2xl border border-chalk/10">
        {mode === '2d' ? (
          <>
            <Board2D
              elements={elements}
            ...
            />
            <div className="absolute bottom-3 right-3 flex items-center gap-1 rounded-full border border-chalk/15 bg-blackboard/80 px-2 py-1 font-mono text-xs text-chalk/80">
              <button
                type="button"
                aria-label="Zoom out"
                className="px-2"
                onClick={() => changeViewport(zoomAround(viewport, { x: 400, y: 300 }, 1 / 1.25))}
              >
                −
              </button>
              <button
              ...
              <button
                ...
                aria-label="Zoom in"
                className="px-2"
                onClick={() => changeViewport(zoomAround(viewport, { x: 400, y: 300 }, 1.25))}
              >
                +
              </button>
          ...
          <>
            ...
            <div className="absolute left-3 top-3 flex flex-col gap-2 rounded-xl border border-chalk/15 bg-blackboard/80 p-3 font-sans text-xs text-chalk/80">
          ...
          <div className="flex h-full flex-col items-center justify-center gap-3 px-8 text-center">
            ...
            <p className="max-w-lg font-display text-2xl">
              Four dimensions: space you can walk into, and time you can scrub.
            </p>
            <p className="max-w-lg font-sans text-sm text-chalk/70">
              The board already stamps every element with its creation time — the fourth axis. The
              experimental 3D view uses it as depth and as a time slider; a navigable 4D spacetime
      ...
      <p className="font-sans text-xs text-dust">
        Speak and your words land at the ochre cursor (click the board with Select to move it). Say
        “draw a circle”, “add an arrow”, “draw a box” or “undo”. Scroll to zoom, hold Space to pan.
      </p>
    </div>
  );
}
