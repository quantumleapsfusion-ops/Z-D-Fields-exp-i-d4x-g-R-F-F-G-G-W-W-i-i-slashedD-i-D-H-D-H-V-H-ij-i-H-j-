...
export default function Board3D({
  ...
  useEffect(() => {
    ...
    const camera = new THREE.PerspectiveCamera(50, el.clientWidth / el.clientHeight, 1, 1e6);
    ...
    if (bounds.isEmpty()) bounds.set(new THREE.Vector2(-400, -300), new THREE.Vector2(400, 300));
    const center = bounds.getCenter(new THREE.Vector2());
    const extent = Math.max(bounds.max.x - bounds.min.x, bounds.max.y - bounds.min.y, 400);
    ...
    for (const e of elements) {
      ...
      if (e.type === 'text') {
        ...
        sprite.position.copy(v(e.x + (canvas.width * k) / 2, e.y + (canvas.height * k) / 2, z));
        object = sprite;
      } else {
        ...
        if (e.type === 'rect') {
          ...
          pts = [];
          for (let i = 0; i < e.points.length; i += 2) pts.push(v(e.points[i], e.points[i + 1], z));
        }
  ...
  useEffect(() => {
    for (const { object, createdAt } of objects.current) object.visible = createdAt <= until;
