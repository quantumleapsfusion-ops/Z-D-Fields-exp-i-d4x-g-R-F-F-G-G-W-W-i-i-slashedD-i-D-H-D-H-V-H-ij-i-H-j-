...
export default function Board2D({
  ...
  function handleUp() {
    ...
    const tiny =
      ...
      (d.type === "line" &&
        Math.hypot(d.points[2] - d.points[0], d.points[3] - d.points[1]) <
          2 / viewport.scale) ||
      (d.type === "rect" &&
        (d.width < 2 / viewport.scale || d.height < 2 / viewport.scale)) ||
      (d.type === "ellipse" &&
        (d.radiusX < 1 / viewport.scale || d.radiusY < 1 / viewport.scale));
